...
import { Gamepad2, Shield, Zap, Headphones, ArrowRight, ShieldCheck, Clock, Sparkles } from "lucide-react";
...
export default async function HomePage() {
  ...
  const products = await prisma.product.findMany({
    where: { status: "active" }, // Apenas produtos ativos aparecem no catálogo
    include: { seller: true },
    ...
    <div className="min-h-screen bg-[#0a0e17] text-white">
      ...
      <section className="max-w-5xl mx-auto px-4 py-16 text-center space-y-6 relative">
        ...
        <div className="flex justify-center gap-3">
          <Link href="#vitrine" className="bg-[#00e676] hover:bg-emerald-400 text-black font-extrabold px-6 py-3 rounded-xl text-xs transition">
            Explorar anúncios
          </Link>
          <Link href="/dashboard/profile" className="bg-neutral-900 border border-neutral-800 px-6 py-3 rounded-xl text-xs font-bold transition">
            Quero vender
          </Link>
        </div>
      </section>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {categories.map((cat, i) => (
            <div key={i} className="bg-neutral-900/30 border border-neutral-800/80 rounded-xl p-3 flex items-center gap-2">
              <div className={`w-5 h-5 rounded bg-gradient-to-r ${cat.color} shrink-0`} />
      ...
      <section id="vitrine" className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        {products.length === 0 ? (
          <p className="text-xs text-neutral-500 text-center py-12 border border-dashed border-neutral-800 rounded-xl">Nenhum produto ativo à venda no momento.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {products.map((p) => (
              <div key={p.id} className="bg-neutral-900/30 border border-neutral-800 rounded-2xl p-5 flex flex-col justify-between hover:border-emerald-500/30 transition duration-200">
                ...
                {/* LINK ENVOLVENDO O CONTEÚDO DO CARD */}
                <Link href={`/products/${p.id}`} className="space-y-3 cursor-pointer group">
